module ApplicationHelper
  ...
  def rating_for(rateable_obj, dimension=nil, options={})
    ...
    star_half    = options[:star_half]    || image_path('star-half.png')
    cancel       = options[:cancel]       || false
    cancel_place = options[:cancel_place] || 'right'
    cancel_hint  = options[:cancel_hint]  || 'Оменить рейтинг'
    cancel_on    = options[:cancel_on]    || image_path('cancel-on.png')
    cancel_off   = options[:cancel_off]   || image_path('cancel-off.png')
    noRatedMsg   = options[:noRatedMsg]   || 'Нет ни одной оценки!'
    # round        = options[:round]        || { down: .26, full: .6, up: .76 }
    space        = options[:space]        || false
    single       = options[:single]       || false
    target       = options[:target]       || ''
  ...
  def sum_raiting(obj, user)
    rates = Rate.where(rater_id: user.id, rateable: obj).pluck(:stars)
    if rates.size > 0
      score = (rates.sum / rates.size).round(2)
    else
      score = 0
    end
    return score
  ...
  def total_rating_for_user(rateable_obj, rating_user, options = {})
    @object = rateable_obj
    @user   = rating_user
    stars   = sum_raiting(@object, @user)
